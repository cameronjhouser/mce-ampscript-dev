...
function rebuildDEDropdown() {
  ...
  deClearBtn.style.display = deEngine.getUploadedNames().length ? 'inline-block' : 'none';
}
...
function updateSubNav() {
  ...
  subClearBtn.style.display = total > 0 ? 'inline-block' : 'none';
}
...
const REF = {
  ...
  'string': [
    ...
    { name: 'Char()',                    syntax: "Char(65)",                   desc: 'Character from ASCII code (e.g. 65 = A)',     snippet: "%%=Char(65)=%%" },
  ...
  'date': [
    ...
    { name: 'StringToDate()',          syntax: "StringToDate('2026-01-15')",         desc: 'Alias for DateParse() — convert string to date value',      snippet: "%%=StringToDate('2026-01-15')=%%" },
    { name: 'FormatDate()',            syntax: "FormatDate(Now(),'MM/DD/YYYY')",     desc: 'Format a date — alias for Format() in the Date & Time category', snippet: "%%=FormatDate(Now(),'MM/DD/YYYY')=%%" },
  ...
  'logic': [
    ...
    { name: 'OutputLine()',      syntax: "OutputLine(@var)",          desc: 'Output value followed by a line break',          snippet: "%%[ OutputLine(@var) ]%%" },
  ...
  'simulated': [
    ...
    { name: 'ContentBlockByName()',  syntax: "ContentBlockByName('header')",           desc: 'HTML snippet by name — matches content block Name field above',  snippet: "%%=ContentBlockByName('header')=%%" },
    { name: 'ContentBlockByKey()',   syntax: "ContentBlockByKey('key')",              desc: 'HTML snippet by key — matches content block Key field above',   snippet: "%%=ContentBlockByKey('key')=%%" },
    { name: 'ContentBlockById()',    syntax: "ContentBlockById(5593491)",              desc: 'HTML snippet by numeric ID — matches content block ID field',   snippet: "%%=ContentBlockById(5593491)=%%" },
    ...
    { name: 'HttpGet()',             syntax: "HttpGet('https://api.example.com')",     desc: 'Mock HTTP GET — configure in Mock HTTP tab',                    snippet: "%%=HttpGet('https://api.example.com/data')=%%" },
  ...
  'unsupported': [
    ...
    { name: 'Redirect()',                            syntax: "Redirect(@url)",           desc: 'Redirect the browser to a URL (CloudPages) — requires live CloudPages runtime',                                                   unsupported: true },
    { name: 'RequestHeader()',                       syntax: "RequestHeader('header')",  desc: 'Reads a live HTTP request header — requires server context, not available in browser',                                            unsupported: true },
    { name: 'HttpPost2()',                           syntax: "HttpPost2(@url,@ct,@body)", desc: 'HTTP POST returning status code and body separately — requires server-side network access',                                      unsupported: true },
    ...
    { name: 'WAT() / WATP()',                        syntax: "WAT()",                    desc: 'Web analytics tracking pixels — inject open-tracking HTML; require live MCE email context',                                      unsupported: true },
    { name: 'BeginImpressionRegion() / EndImpressionRegion()', syntax: "BeginImpressionRegion('name')", desc: 'Analytics impression regions — require live MCE tracking service',                                               unsupported: true },
    ...
    { name: 'UpsertContact()',                       syntax: "UpsertContact(@sub,1,'f',@v)", desc: 'Create or update a contact record — requires live MCE Contact Builder',                                                    unsupported: true },
    { name: 'MicrositeURL() / LiveContentMicrositeURL()', syntax: "MicrositeURL('name')", desc: 'Generate microsite URL variants — require live MCE org and published pages',                                                  unsupported: true },
    ...
    { name: 'InvokeCreate() / InvokeRetrieve() / InvokeUpdate() / InvokeDelete()', syntax: "InvokeRetrieve(...)", desc: 'SOAP API calls — require live SFMC credentials and cannot run in a browser context',                  unsupported: true },
